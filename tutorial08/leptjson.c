    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            for (i = 0; i < lhs->u.o.size; i++) {
                j = lept_find_object_index(lhs, lept_get_object_key(rhs, i), lept_get_object_key_length(rhs,i));
                if (LEPT_KEY_NOT_EXIST == j)
                    return 0;
                if (!lept_is_equal(&lhs->u.o.m[j].v, &rhs->u.o.m[i].v))
                    return 0;
            }
            return 1;
        default:
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
    ...
    if (index + count == v->u.a.size)
    {
        ...
        for (i = index + count; i < v->u.a.size; ++index, ++i){
            lept_move(&v->u.a.e[index], &v->u.a.e[i]);
            lept_free(&v->u.a.e[i]);
        }
    }
    v->u.a.size -= count;
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    lept_value* p;
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    p = lept_find_object_value(v, key, klen);
    if (NULL == p) {
